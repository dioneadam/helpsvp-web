...
export class MedicamentoService {
  ...
  base_url = HOST + '/v1/medicamento/';
  constructor(private http: HttpClient, private messageService: MessageService) { }

  getResidents() : Observable<Medicamento[]> {

    let params = new HttpParams();
              
    return this.http.get<Medicamento[]>(this.base_url, {params: params})
      .pipe(
        tap(residents => {
          console.log('Fetched residents');
          console.log(residents);
          this.messageService.add("Success","Fetched /v1/medicamento/");
        }, err => {
  ...
  updateResident(resident : Medicamento) {
  ...
  saveResident (resident : Medicamento) {
    return this.http.post<Medicamento>(this.base_url, resident)
      .pipe(
        tap(person => {
  ...
  deleteResident(resident : Medicamento) {
